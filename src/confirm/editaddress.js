...
class EditAddress extends Component {
  ...
  componentDidMount() {
    console.log('address',this.props.navigation.state.params);
    if(this.props.navigation.state.params.address_id){
      this.setState({ firstname: this.props.navigation.state.params.firstname });
      this.setState({ lastname: this.props.navigation.state.params.lastname });
      this.setState({ address_1: this.props.navigation.state.params.address_1 });
      this.setState({ address_2: this.props.navigation.state.params.address_2 });
      this.setState({ address_id: this.props.navigation.state.params.address_id });
      this.setState({ city: this.props.navigation.state.params.city });
      this.setState({ postcode: this.props.navigation.state.params.postcode });
      this.setState({ country_id: this.props.navigation.state.params.country_id });
      this.setState({ zone_id: this.props.navigation.state.params.zone_id });
      if(this.props.navigation.state.params.address_1==this.props.navigation.state.params.address_2){
      ...
      <View style={styles.container}>
        <ScrollView>
          ...
          <View style={styles.row}>
            ...
            <FormInput onChangeText={(firstname) => this.setState({ firstname })} placeholder="Enter your first name" value={this.state.firstname}/>

            <FormLabel>Last Name</FormLabel>
            <FormInput onChangeText={(lastname) => this.setState({ lastname })} placeholder="Enter your last name" value={this.state.lastname} />
            ...
            <FormInput onChangeText={(address_1) => this.setState({ address_1 })} placeholder="Enter address" value={this.state.address_1} />
            ...
            <FormInput value={this.state.address_2} onChangeText={(address_2) => this.setState({ address_2 })} placeholder="Enter address" value={this.state.address_2} />
            <FormLabel>City</FormLabel>
            <FormInput onChangeText={(city) => this.setState({ city })} placeholder="Enter City"  value={this.state.city}/>
            <FormLabel>Zip</FormLabel>
            <FormInput keyboardType={'numeric'} onChangeText={(postcode) => this.setState({ postcode })} placeholder="Enter zip code" value={this.state.postcode} />

            <View style={{ paddingLeft: 20 }}>{this.country()}</View>
            <View style={{ paddingLeft: 20 }}>{this.doState()}</View>

            <Button onPress={() => this.submit()} raised title='Update' icon={{ name: 'send' }} style={{ width: '100%' }} backgroundColor="#51c0c3" />
          </View>
        </ScrollView>
      </View>
    );
